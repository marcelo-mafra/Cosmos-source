...
uses Winapi.Windows, System.SysUtils, System.Classes, Vcl.SvcMgr, Datasnap.DSServer,
  Datasnap.DSTCPServerTransport, Datasnap.DSCommonServer,Datasnap.DSAuth,
  IPPeerServer, System.Generics.Collections, Datasnap.DSSession,
...
type
  ...
  TCosmosConferenciasService = class(TService)
...
var
  CosmosConferenciasService: TCosmosConferenciasService;
...
begin
  CosmosConferenciasService.Controller(CtrlCode);
end;
...
procedure TCosmosConferenciasService.DSServerApplicationClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosApplicationServer;
end;

procedure TCosmosConferenciasService.DSServerAtividadesClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMSecAtividadesServerMethods;
end;

procedure TCosmosConferenciasService.DSServerLogradourosClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosServerLogradouros;
end;

procedure TCosmosConferenciasService.DSTCPServerTransportConnect(
...
begin
 ...
 //Registra qual o servidor Cosmos que está usando a classe TCosmosConferenciasService.
...
procedure TCosmosConferenciasService.DSTCPServerTransportDisconnect(
  Event: TDSTCPDisconnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
function TCosmosConferenciasService.GetServiceController: TServiceController;
begin
  Result := ServiceController;
end;

function TCosmosConferenciasService.DoContinue: Boolean;
begin
  Result := inherited;
  DSServerConferencias.Start;
end;

procedure TCosmosConferenciasService.DoInterrogate;
begin
  inherited;
end;

function TCosmosConferenciasService.DoPause: Boolean;
begin
  DSServerConferencias.Stop;
  Result := inherited;
end;

function TCosmosConferenciasService.DoStop: Boolean;
begin
  DSServerConferencias.Stop;
  Result := inherited;
end;

procedure TCosmosConferenciasService.DSServerConferenciasClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosConferenciasMethods;
end;

procedure TCosmosConferenciasService.DSServerConferenciasConnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
procedure TCosmosConferenciasService.DSServerConferenciasDisconnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
procedure TCosmosConferenciasService.DSServerConferenciasError(
...
procedure TCosmosConferenciasService.DSServerConferenciasPrepare(
  DSPrepareEventObject: TDSPrepareEventObject);
var
 Info: string;
 AContextInfo: TStringList;
...
function TCosmosConferenciasService.DSServerConferenciasTrace(
...
procedure TCosmosConferenciasService.LoadMethodsAuthorizations;
var
...
procedure TCosmosConferenciasService.LoadServerConfigurations;
 var
...
{Carregas as configurações do servidor em execução. Estas configurações envolvem
 ...
 try
  ...
  DSTCPServerTransport.Port := AFile.ReadInteger('GCON', 'TCPConnection.Port', 2100);
  DSTCPServerTransport.BufferKBSize := AFile.ReadInteger('GCON', 'TCPConnection.BufferKbSize', 32);
  DSTCPServerTransport.PoolSize := AFile.ReadInteger('GCON', 'TCPConnection.PoolSize', 10);
  DSTCPServerTransport.MaxThreads := AFile.ReadInteger('GCON', 'TCPConnection.MaxThreads', 0);

  //Opções de KeepAlive.
  if AFile.ReadBoolean('GCON', 'TCPConnection.KeepAliveEnablement', False) then
   begin
     DSTCPServerTransport.KeepAliveEnablement := kaEnabled;
     DSTCPServerTransport.KeepAliveTime := AFile.ReadInteger('GCON', 'TCPConnection.KeepAliveTime', 300000);
     DSTCPServerTransport.KeepAliveInterval := AFile.ReadInteger('GCON', 'TCPConnection.KeepAliveInterval', 100);
   end;
...
procedure TCosmosConferenciasService.ServiceAfterInstall(Sender: TService);
const
 sServiceDescription = 'Implementa o servidor Cosmos Gestor de Conferências, da ' +
 'suíte Cosmos. Esse serviço disponibiliza as funcionalidades relacionadas ' +
 'aos processos de gestão de conferências da Escola Internacional da Rosacruz Áurea.';//do not localize!
var
...
procedure TCosmosConferenciasService.ServiceCreate(Sender: TObject);
begin
...
procedure TCosmosConferenciasService.ServiceDestroy(Sender: TObject);
begin
  FDSServer := nil;
...
procedure TCosmosConferenciasService.ServiceShutdown(Sender: TService);
begin
 if Assigned(DMServerDataAcess) then
  DMServerDataAcess.ConnectionPool.ClearAll;
end;

procedure TCosmosConferenciasService.ServiceStart(Sender: TService;
  var Started: Boolean);
begin
  DSServerConferencias.Start;
end;
...
procedure TCosmosConferenciasService.DSAuthenticationManagerUserAuthenticate(
...
procedure TCosmosConferenciasService.DSAuthenticationManagerUserAuthorize(
