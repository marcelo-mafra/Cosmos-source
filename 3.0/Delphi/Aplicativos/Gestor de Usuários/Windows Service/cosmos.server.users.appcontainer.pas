...
uses Winapi.Windows, System.SysUtils, System.Classes, Vcl.SvcMgr, Datasnap.DSServer,
  ...
  Data.DBXCommon, cosmos.classes.logs, cosmos.classes.persistence.ini,
  cosmos.classes.persistence.registry, Data.DBCommonTypes;
...
type
  ...
  TCosmosUsuariosService = class(TService)
...
var
  CosmosUsuariosService: TCosmosUsuariosService;
...
begin
  CosmosUsuariosService.Controller(CtrlCode);
end;
...
procedure TCosmosUsuariosService.DSServerApplicationClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosApplicationServer;
end;

procedure TCosmosUsuariosService.DSServerUsuariosConnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
...
procedure TCosmosUsuariosService.DSServerUsuariosDisconnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
procedure TCosmosUsuariosService.DSServerUsuariosError(
...
procedure TCosmosUsuariosService.DSServerUsuariosPrepare(
  DSPrepareEventObject: TDSPrepareEventObject);
var
 Info: string;
 AContextInfo: TStringList;
...
function TCosmosUsuariosService.DSServerUsuariosTrace(
...
procedure TCosmosUsuariosService.DSTCPServerTransportConnect(
...
begin
 ...
 //Registra qual o servidor Cosmos que está usando a classe TCosmosUsuariosService.
...
procedure TCosmosUsuariosService.DSTCPServerTransportDisconnect(
  Event: TDSTCPDisconnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
function TCosmosUsuariosService.GetServiceController: TServiceController;
begin
  Result := ServiceController;
end;

procedure TCosmosUsuariosService.DSServerUsuariosClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
 PersistentClass := TDMUserMethods;
end;

procedure TCosmosUsuariosService.LoadMethodsAuthorizations;
var
...
procedure TCosmosUsuariosService.LoadServerConfigurations;
 var
...
{Carregas as configurações do servidor em execução. Estas configurações envolvem
 ...
 try
  ...
  DSTCPServerTransport.Port := AFile.ReadInteger('GUSU', 'TCPConnection.Port', 2100);
  DSTCPServerTransport.BufferKBSize := AFile.ReadInteger('GUSU', 'TCPConnection.BufferKbSize', 32);
  DSTCPServerTransport.PoolSize := AFile.ReadInteger('GUSU', 'TCPConnection.PoolSize', 10);
  DSTCPServerTransport.MaxThreads := AFile.ReadInteger('GUSU', 'TCPConnection.MaxThreads', 0);

  //Opções de KeepAlive.
  if AFile.ReadBoolean('GUSU', 'TCPConnection.KeepAliveEnablement', False) then
   begin
     DSTCPServerTransport.KeepAliveEnablement := kaEnabled;
     DSTCPServerTransport.KeepAliveTime := AFile.ReadInteger('GUSU', 'TCPConnection.KeepAliveTime', 300000);
     DSTCPServerTransport.KeepAliveInterval := AFile.ReadInteger('GUSU', 'TCPConnection.KeepAliveInterval', 100);
   end;
...
procedure TCosmosUsuariosService.ServiceAfterInstall(Sender: TService);
const
 sServiceDescription = 'Implementa o servidor Cosmos Gestor de Usuários, da ' +
 'suíte Cosmos. Esse serviço disponibiliza acesso às funcionalidades relacionadas ' +
 'a permissões e gestão de usuários dos aplicativos da suíte Cosmos.';//do not localize!
var
...
procedure TCosmosUsuariosService.ServiceCreate(Sender: TObject);
begin
...
procedure TCosmosUsuariosService.ServiceDestroy(Sender: TObject);
begin
  FDSServer := nil;
...
procedure TCosmosUsuariosService.ServiceShutdown(Sender: TService);
begin
 if Assigned(DMServerDataAcess) then
  DMServerDataAcess.ConnectionPool.ClearAll;
end;

procedure TCosmosUsuariosService.ServiceStart(Sender: TService;
  var Started: Boolean);
begin
  DSServerUsuarios.Start;
end;
...
function TCosmosUsuariosService.DoContinue: Boolean;
begin
  Result := inherited;
  DSServerUsuarios.Start;
end;

procedure TCosmosUsuariosService.DoInterrogate;
begin
  inherited;
end;

function TCosmosUsuariosService.DoPause: Boolean;
begin
  DSServerUsuarios.Stop;
  Result := inherited;
end;

function TCosmosUsuariosService.DoStop: Boolean;
begin
  DSServerUsuarios.Stop;
  Result := inherited;
end;

procedure TCosmosUsuariosService.DSAuthenticationManagerUserAuthenticate(
...
procedure TCosmosUsuariosService.DSAuthenticationManagerUserAuthorize(
