...
uses
  ...
  cosmos.classes.logs.controller, Cosmos.Framework.Interfaces.Root, Cosmos.Framework.Interfaces.DataAcess,
  Cosmos.Framework.Interfaces.Utils, Cosmos.Framework.Interfaces.Dialogs,
  System.Contnrs, Vcl.CustomizeDlg, cosmos.framework.forms.wizards,
  ...
  Vcl.Touch.GestureMgr, Data.DBXCommon, cosmos.classes.cripter, IdStack, System.ImageList,
...
type
 ...
 /// <summary>Pode ser usada com TConnectionThread.</summary>
  ...
  private
    ...
    procedure RegisterLogFileName(NewFileName: string);
...
begin
 //Encerra a conexão com o servidor remoto.
 try
...
AForm: TForm;
begin
 {Acessa a biblioteca de cadastro de logradouros dos Correios.}
...
begin
//Carrega a ferramenta Central de Pesquisas.
...
begin
 //Chama o sistema de ajuda - visão de índices de busca.
...
begin
 //Chama o sistema de ajuda - visão de tópicos de ajuda.
...
begin
 ...
 //Salva o estilo no arquivo de configurações.
 WriteStartupOptions;
end;
...
{Inicia a operação de autenticação do usuário. Ela é um loop que continuará
 ...
 FAbortedLogin := False;
 OutPutDebugString(PWideChar('Iniciarn login'));
...
begin
 ...
 if TAction(Sender).Checked then
   FrmModernMenu.Width := 185; //Largura padrão.
end;
...
begin
//Acessa a janela de registro do aplicativo. Esse recurso está depreciado.
...
begin
 //Registra um log quando ocorrer uma exceção não tratada pela aplicação.
  ...
  begin
    ...
    MessageDlg(TCosmosErrorMsg.ConnectionLost + '#13#10#13#10' + E.Message, mtError, [mbOK], 0);
    IRemoteCon.CloseConnection;
    Abort;
  end;
...
var
 ...
 ANode: IXMLNode;
begin
{Carrega as configurações de registro de logs da aplicação em execução. Nesse método
usa-se a função "VarToStr" para fazer a cnversão explícita de uma Olevariant para
uma string. Isso porque, na leitura do arquivo XML de configurações, caso a string
esteja vazia, será retornado o valor "null", que não pode ser atribuído a um tipo
string.}
 FLogEvents := [];

 AXMLDoc := self.CreateXMLDocument;
 AXMLDoc.FileName  := CosmosAppInfo.GetCommonConfigurationsFile;
 OutputDebugString(PWideChar(AXMLDoc.FileName));
 ...
 try
  ...
  {VarToStr usada para conversão explícita. Ver cometário no início do método.}
  FLogsFilesPath := VarToStr(ANode.Attributes['LogsFilesPath']); //do not localize!
  FMaxFileSize := ANode.Attributes['MaxFileSize']; //default 512kb 524288 //do not localize!

  if Trim(FLogsFilesPath) = '' then
   Exit;
  ...
  if (ANode <> nil) then
    begin
      ...
      {VarToStr usada para conversão explícita. Ver cometário no início do método.}
      CurrentLogFile := VarToStr(ANode.Attributes['CurrentFile']); //do not localize!
  ...
  on E: Exception do
   begin
     ...
     DlgMessage.ErrorMessage(TCosmosTitles.SystemFailure, Format(TCosmosErrorMsg.InitializationLogs , [self.ClassName]) + #13 + E.Message);
     Application.Terminate;
   end;
 end;
end;
...
//Lê as configurações de inicialização.
 if not FStarted then
  begin
   ...
   //Lê as configurações de elementos visuais da aplicação.
   AXMLDoc := self.CreateXMLDocument;
   ...
   //Avalia o registro da aplicação. Depreciado.
...
begin
 ...
 //Result := CreateLogsObject as ICosmosLogs;
...
procedure TFrmMainClientGUI.RegisterLogFileName(NewFileName: string);
var
 AXMLDoc: TXMLDocument;
 ANode: IXMLNode;
begin
{Carrega as configurações de registro de logs da aplicação em execução.}
 AXMLDoc := self.CreateXMLDocument;
 AXMLDoc.FileName  := CosmosAppInfo.GetCommonConfigurationsFile;
 OutputDebugString(PWideChar('RegisterLogFileName: ' + AXMLDoc.FileName));
