...
//Gera um novo nome de arquivo de log.
 ...
 Result := ANewFile;
 OutputDebugString(PWideChar('TTextFileLog New: ' + ANewFile));
end;
...
{Escre os dados no arquivo de log.}
 OutputDebugString(PWideChar('TTextFileLog WriteMessage: início.'));
 ...
 try
  if (CurrentFile.Trim = '') then //Checa se existe um arquivo corrente.
   begin  //Não existe.
     ...
     OutputDebugString(PWideChar('TTextFileLog WriteMessage: ' + CurrentFile));
     AFileWriter.SaveToFile(CurrentFile);
   end;

  if not FileExists(CurrentFile) then //Arquivo corrente não existe.
   begin
